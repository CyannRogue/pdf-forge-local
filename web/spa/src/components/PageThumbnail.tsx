export function PageThumbnail({ src, page, rotation = 0, onDelete, onRotate }: { src: string; page: number; rotation?: number; onDelete?: () => void; onRotate?: () => void }) {
  return (
    <div className="relative bg-[#161a22] border border-[#2a3142] rounded-lg p-2">
      <img src={src} alt={`Page ${page}`} className="w-full h-auto rounded" style={{ transform: `rotate(${rotation}deg)` }} />
